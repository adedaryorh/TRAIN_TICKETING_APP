...
public interface TrainBookingDao extends JpaRepository<TrainBooking, Integer> {
	...
	TrainBooking findByUserIdAndScheduleTrainIdAndTrainSeatIdAndStatusOrderByIdDesc(
			int userId,
			int scheduleTrainId,
			int trainSeatId,
			int status
	...
	List<TrainBooking> findByScheduleTrainIdAndStatusNotOrderByIdDesc(int scheduleTrainId,
																	  int status);

	List<TrainBooking> findByScheduleTrainIdAndStatusIn(int scheduleTrainId,
