...
import com.ticketbooking.book.external_Resources.TrainService;
import com.ticketbooking.book.external_Resources.UserService;
...
public class TrainBookingResource {
	...
	public ResponseEntity<CommonApiResponse> bookTrain(BookTrainRequestDto bookingRequest) {
		...
		response.setResponseMessage(
				"Your Booking ID is [ " + bookingId + " ] , Please check the booking status on dashboard");
		response.setSuccess(true);
	...
	public ResponseEntity<CommonApiResponse> addScheduledTrainTickets(
		...
		for (TrainSeat seat : addTicketRequest.getTrainSeats()) {
			...
			TrainBooking booking = TrainBooking.builder()
					.scheduleTrainId(addTicketRequest.getScheduleTrainId())
